...
const serverSchema = createUserSchema
    .refine(
        async (data) => {
            const existingUsername = await db.select().from(userSchema).where(eq(userSchema.username, data.username));
    ...
    .refine(
        async (data) => {
            const existingEmail = await db.select().from(userSchema).where(eq(userSchema.email, data.email));
